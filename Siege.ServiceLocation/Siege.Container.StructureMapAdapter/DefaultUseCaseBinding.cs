...
namespace Siege.ServiceLocation.StructureMapAdapter
{
    public class DefaultUseCaseBinding<TService> : IDefaultUseCaseBinding<TService>
    {
        ...
        {
			if (container.Model.HasDefaultImplementationFor(useCase.GetBoundType())) return;
        ...
        public void BindInstance(IInstanceUseCase useCase, IFactoryFetcher locator)
		{
            ...
            registry.ForRequestedType<TService>().CacheBy(InstanceScope.PerRequest).AddInstances(ex => ex.IsThis((TService)useCase.GetBinding()));
